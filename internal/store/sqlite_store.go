...
func (s *SQLiteStore) SaveScan(scan domain.ScanRun) error {
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}
	...
	if _, err := tx.Exec(`insert into scans(id, name, started_at, completed_at, targets_json, errors_json)
		values(?, ?, ?, ?, ?, ?)
	...
	for _, cert := range scan.Certificates {
		if err := saveCertificate(tx, cert); err != nil {
			return err
		}
		if _, err := tx.Exec(`insert or ignore into scan_certificates(scan_id, certificate_id) values(?, ?)`, scan.ID, cert.ID); err != nil {
			return err
		}
	}
	for _, risk := range scan.Risks {
		if err := saveRisk(tx, risk); err != nil {
			return err
		}
		if _, err := tx.Exec(`insert or ignore into scan_risks(scan_id, risk_id) values(?, ?)`, scan.ID, risk.ID); err != nil {
			return err
		}
	}
	return tx.Commit()
...
func (s *SQLiteStore) SaveReport(report domain.HandoffReport) error {
	...
	_, err = s.db.Exec(`insert into reports(id, service_id, certificate_id, summary, risks_json, handoff_checklist_json, renewal_steps_json, evidence_ids_json, created_at)
		values(?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (s *SQLiteStore) ListCertificates() ([]domain.Certificate, error) {
...
func (s *SQLiteStore) ListRisks() ([]domain.RiskFinding, error) {
...
func (s *SQLiteStore) ListReports() ([]domain.HandoffReport, error) {
...
func (s *SQLiteStore) ListScans() ([]domain.ScanRun, error) {
...
func saveCertificate(tx *sql.Tx, cert domain.Certificate) error {
	...
	_, err = tx.Exec(`insert into certificates(id, fingerprint_sha256, serial_number, subject_common_name, issuer_common_name, not_before, not_after, dns_names_json, ip_addresses_json, public_key_algorithm, signature_algorithm, source, endpoint, service_id, environment, owner_team, renewal_method, tags_json, first_seen_at, last_seen_at)
		values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func saveRisk(tx *sql.Tx, risk domain.RiskFinding) error {
	evidenceJSON, err := marshal(risk.Evidence)
	if err != nil {
		return err
	}
	_, err = tx.Exec(`insert into risks(id, certificate_id, severity, category, title, evidence_json, status, created_at)
		values(?, ?, ?, ?, ?, ?, ?, ?)
